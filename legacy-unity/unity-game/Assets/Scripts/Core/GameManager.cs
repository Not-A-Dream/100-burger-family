...
public class GameManager : MonoBehaviour
{
    ...
    /// </summary>
    public bool ServeBurger()
    {
        var dailyRun = DailyBurgerRunManager.I;
        if (!dailyRun.TryCompleteBurger(out float activeSeconds))
        {
            state.lastMessage = "아직 전달할 주문이 준비되지 않았어요.";
            state.lastServeSucceeded = false;
            NotifyChanged();
            OnFeedback?.Invoke(state.lastMessage, false);
            return false;
        }

        state.burgerCount++;
        state.lastServeSucceeded = true;
        UpdateStreak();
        CheckAchievements();
        state.lastMessage = $"버거 완성! 기록 {DailyBurgerRunManager.FormatSeconds(activeSeconds)} / 누적 {state.burgerCount}개";
        NotifyChanged();
        OnFeedback?.Invoke("주문 성공! 맛있는 버거가 완성됐어요.", true);
        Debug.Log($"[Game] +1 {state.burgerCount} / {CurrentStreak}d / {DailyBurgerRunManager.FormatSeconds(activeSeconds)}");
        return true;
    }

    public void RegisterServeFailure(string reason)
    {
        state.failureCount++;
        state.lastServeSucceeded = false;
        state.lastMessage = reason;
        DailyBurgerRunManager.I.RegisterOrderFailure();
        NotifyChanged();
        OnFeedback?.Invoke(reason, false);
        Debug.Log($"[Game] Fail: {reason}");
    }

    public void PostFeedback(string message, bool success)
    {
        if (string.IsNullOrWhiteSpace(message)) return;
        state.lastMessage = message;
        NotifyChanged();
        OnFeedback?.Invoke(message, success);
    }
