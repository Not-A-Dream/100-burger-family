...
public class PlayerController : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        if (_hand != null)
            _hand.OnChanged += HandleHandChanged;
        if (_rb != null)
        {
    ...
    void Start()
    {
        HandleHandChanged(_hand != null ? _hand.heldItem : IngredientType.None);
    }
    ...
    void Update()
    {
        ...
        if (kb.qKey.wasPressedThisFrame)
        {
            if (_hand != null && _hand.Has(IngredientType.WateringCan))
                ...
                _hand?.DropToWorld(transform);
            return;
        }

        // ── E / Space → 대상 오브젝트 상호작용 ─────────────
    ...
    void RefreshNearestInteractable()
    {
        ...
        foreach (var inter in FindObjectsByType<Interactable>(FindObjectsInactive.Exclude, FindObjectsSortMode.None))
        {
            ...
            if (d2 < best2 && inter.IsInRange(transform.position))
            {
                nearest = inter;
                best2 = d2;
            }
    ...
    void HandleHandChanged(IngredientType itemType)
    {
        OnItemChanged?.Invoke(itemType == IngredientType.None ? "" : IngredientNames.Korean(itemType));
    }
}
