...
public class FarmStation : Interactable
{
    ...
    public override string GetPrompt()
    {
        ...
        return stage switch
        {
            FarmStage.Idle       => $"{name} 재배기\n[E] 씨앗 심기\n새싹까지 {FormatRemaining(DailyBurgerRunManager.SeedToSproutSeconds)}",
            FarmStage.Seeded     => $"새싹 기다리는 중...\n남은 시간 {FormatRemaining(remaining)}",
            FarmStage.NeedsWater => $"{name} 새싹이 났어요!\n양수기를 들고 [E] 물 주기\n열매까지 {FormatRemaining(DailyBurgerRunManager.WaterToFruitSeconds)}",
    ...
    public override void Interact(PlayerHand hand)
    {
        ...
        switch (Stage)
        {
            ...
            case FarmStage.Ready:
                ...
                {
                    Debug.Log("[Farm] 손이 찼어요");
                    return;
                }

                if (run.TryHarvest(cropType, out _))
                {
                    hand.PickUp(cropType);
