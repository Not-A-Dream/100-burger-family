...
public class PlayerHand : MonoBehaviour
{
    ...
    public event Action<IngredientType> OnChanged;
    ...
    public bool TryPickUp(IngredientType type)
    {
        if (type == IngredientType.None || !IsEmpty)
            return false;

        heldItem = type;
        RefreshHeldVisual();
        OnChanged?.Invoke(heldItem);
    ...
    public bool DropToWorld(Transform owner)
    {
        if (owner == null || IsEmpty || heldItem == IngredientType.WateringCan)
            return false;

        DroppedIngredient.Spawn(heldItem, owner);
        Drop();
        return true;
    }
    ...
    {
        AddBox(parent, "HeldItem", Vector3.zero, new Vector3(0.24f, 0.18f, 0.24f), GetColor(type));
    }
    ...
    public static Color GetColor(IngredientType type)
    {
        return type switch
        {
            ...
            IngredientType.AssembledBurger => new Color(0.93f, 0.62f, 0.25f),
